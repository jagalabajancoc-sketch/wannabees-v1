...
if ($room_id <= 0 || !in_array($payment_method, ['GCASH', 'CASH'])) {
...
// Verify room_id matches session rental
...
try {
    ...
    $stmt->bind_param('iiiss', $order_id, $user_id_log, $role_id_log, $meta, $now);
    $stmt->execute();
    $stmt->close();
    ...
    $stmt = $mysqli->prepare("INSERT INTO customer_transactions (rental_id, room_id, transaction_type, reference_id, amount, payment_method, gcash_account_name, gcash_reference_number, status, created_at) VALUES (?, ?, 'ORDER', ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('iiidsssss', $rental_id, $room_id, $order_id, $orderTotal, $payment_method, $gcash_name, $gcash_ref, $tx_status, $now);
    $stmt->execute();
